    ...
    try {
        if (m.mentionedJid.includes(this.user.jid) && m.isGroup) {
            await this.send2Button(m.chat,
                isBanned ? 'BotJr tidak aktif' : banned ? 'kamu dibanned' : '*APAAN NGETAG",ADA BANSOS KAH🗿?*',
                '©JarotBotz',
...
╰═══════════════════════`.trim(), '©zifabotz', 'PAYMENT', '.payment', 'MENU', '.menu', m)
    ...
    if (new Date() * 1 - setting.status > 1000) {
        ...
        await this.setStatus(`time ${uptime} | 🖥️Mode: ${global.opts['self'] ? 'Private' : setting.groupOnly ? 'Hanya Grup' : 'Publik'} |BotJr by.Jarot`).catch(_ => _)
